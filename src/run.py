...
def runNetworkGreedy(type, urgency_mode):
    ...
    if urgency_mode == "distance_asc":
        ...
        load_locations = read_intseries("../network/LoadLocations.txt")
    ...
    for k in range(num_evs):
        ...
        while not feasible:
            ...
            if not cfg.getboolean("general", "network_sensitivity"):
                ...
                if cfg.getboolean("general", "overload_constraints"):
                    slot_overloads = np.unique(np.genfromtxt("../network/LVTest/DI_yr_2/DI_Overloads.CSV", delimiter=',', skip_header=1, usecols=(0,))) / conv.Time(min=resolution).hr - 1
...
def evaluateResults(type):
    ...
    shutil.copytree("../network/LVTest", "../log/" + uuid + "/iter" + str(mc_iter) + "/" + type + "/LVTest")
    ...
    mcoverload_freq = len(np.unique(np.genfromtxt("../network/LVTest/DI_yr_2/DI_Overloads.CSV", delimiter=',', skip_header=1, usecols=(0,)))) / num_slots
...
for mc_iter in range(1, iterations + 1):
    ...
    for hd in households:
        ...
        demand_ts1 = read_floatseries("../timeseries/demand/loadprofile_" + season + "_inh" +
                                     str(hd.inhabitants) + "_" + str(resolution) + "min" + format(hd.day_id_1, "03d") + ".txt")
        hd.day_id_2 = rd.randint(1, hd.id_range)
        demand_ts2 = read_floatseries("../timeseries/demand/loadprofile_" + season + "_inh" +
    ...
    price_ts1 = read_floatseries("../timeseries/price/" + str(resolution) + "min/priceprofile_ukpx_" + str(resolution) + "min" + format(day_id, "04d") + ".txt")
    price_ts2 = read_floatseries("../timeseries/price/" + str(resolution) + "min/priceprofile_ukpx_" + str(resolution) + "min" + format(day_id + 1, "04d") + ".txt")
